...
export const CloudStorageTab: React.FC<CloudStorageTabProps> = ({ onSyncComplete }) => {
  ...
  // 点击添加按钮 - 显示选择类型 Modal
  const handleOpenSelectTypeModal = useCallback(() => {
  ...
  // 选择预设连接器类型
  const handleSelectPresetConnector = useCallback((type: string) => {
  ...
  const handleCancelSelectType = useCallback(() => {
    setIsSelectTypeModalVisible(false);
    setEditingConnector(null);
    setSelectedConnectorType('');
  }, []);
  ...
  // 自定义连接器表单
  const CustomConnectorFormContent: React.FC = () => {
    ...
    <>
      <Spin spinning={loading} style={{ marginTop: 16 }}>
        {connectors.length > 0 ? (
          ...
          >
            <Button type="primary" icon={<PlusOutlined />} onClick={handleOpenSelectTypeModal}>
              {t('cloudStorage.addConnector')}
            </Button>
          </Empty>
        )}
      ...
      <Modal
        ...
        open={isSelectTypeModalVisible}
        onCancel={handleCancelSelectType}
        footer={null}
        width={700}
        destroyOnClose
      >
                ...
                <List
                  ...
                  renderItem={(item) => (
                    <List.Item>
                      <Card
                        hoverable
                        onClick={() => handleSelectPresetConnector(item.type)}
                        title={item.name}
                      >
